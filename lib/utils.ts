...
export type PasswordValidation =
  | { valid: true }
  | { valid: false; reason:
      'empty' | 'too_short' | 'too_long' | 'whitespace' |
      'invalid_char' | 'lower' | 'upper' | 'digit' | 'symbol' |
      'repeat' | 'sequence' | 'common' };
...
export function validatePassword(pw: string, opts: Opts = {}): PasswordValidation {
  const {
    ...
    allowUnicode = false,
    allowSpaces = false,
    disallow = ['password','123456','qwerty','111111','abc123','password1','12345678','admin','letmein'],
  } = opts;
  ...
  const isSeq = (s: string) => {
    ...
    for (const ch of norm) {
      if (!isAlnum(ch)) { up = down = 1; prev = null; continue; }
